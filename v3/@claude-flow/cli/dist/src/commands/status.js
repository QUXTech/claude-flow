...
async function getSystemStatus() {
    try {
        // Get swarm status
        const swarmStatus = await callMCPTool('swarm/status', { includeMetrics: true });
        // Get MCP status
        let mcpStatus = { running: false, port: null, transport: 'stdio' };
        try {
            const mcp = await callMCPTool('mcp/status', {});
            mcpStatus = mcp;
        }
        catch {
            // MCP not running
        ...
        // Get memory status
        const memoryStatus = await callMCPTool('memory/stats', {});
        // Get task status
        const taskStatus = await callMCPTool('task/summary', {});
        return {
            initialized: true,
            running: true,
            swarm: {
...
const agentsCommand = {
    ...
    action: async (ctx) => {
        try {
            const result = await callMCPTool('agent/list', { includeMetrics: true, status: 'all' });
            if (ctx.flags.format === 'json') {
...
const tasksCommand = {
    ...
    action: async (ctx) => {
        try {
            const result = await callMCPTool('task/list', { status: 'all', limit: 50 });
            if (ctx.flags.format === 'json') {
                output.printJson(result);
                return { success: true, data: result };
            }
...
const memoryCommand = {
    ...
    action: async (ctx) => {
        try {
            const result = await callMCPTool('memory/detailed-stats', {});
            if (ctx.flags.format === 'json') {
                output.printJson(result);
                return { success: true, data: result };
            }
